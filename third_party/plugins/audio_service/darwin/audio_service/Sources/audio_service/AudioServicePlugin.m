...
static NSNumber *queueCountValue = nil;
static NSNumber *queueIndexValue = nil;
static NSString *queueCollectionIdentifier = nil;
static NSString * const kAudioServiceQueueIdentifier = @"com.ryanheise.audio_service.queue";
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    if ([@"configure" isEqualToString:call.method]) {
        ...
        NSNumber *oldQueueIndex = queueIndexValue;
        id queueIndexRaw = stateMap[@"queueIndex"];
        if (queueIndexRaw != nil && queueIndexRaw != [NSNull null]) {
            NSInteger idx = [queueIndexRaw integerValue];
            if (queueCountValue != nil) {
                NSInteger maxIndex = MAX(queueCountValue.integerValue - 1, 0);
                if (idx < 0) {
                    idx = 0;
        ...
        BOOL queueIndexChanged = NO;
        if (oldQueueIndex == nil && queueIndexValue == nil) {
            queueIndexChanged = NO;
        } else if (oldQueueIndex != nil && queueIndexValue != nil) {
            queueIndexChanged = ![oldQueueIndex isEqualToNumber:queueIndexValue];
        } else {
            queueIndexChanged = YES;
        }
        updateTime = [NSNumber numberWithLongLong: msSinceEpoch];
        ...
        NSDictionary *args = (NSDictionary *)call.arguments;
        NSArray *rawQueue = args[@"queue"];
        if ([rawQueue isKindOfClass:[NSArray class]] && rawQueue.count > 0) {
            queueCountValue = [NSNumber numberWithUnsignedInteger:rawQueue.count];
            if (queueIndexValue != nil) {
                NSInteger idx = queueIndexValue.integerValue;
                NSInteger maxIndex = MAX(queueCountValue.integerValue - 1, 0);
                if (idx < 0) {
                    queueIndexValue = @(0);
                } else if (idx > maxIndex) {
                    queueIndexValue = @(maxIndex);
                }
            }
            if (queueCollectionIdentifier == nil || queueCollectionIdentifier.length == 0) {
                queueCollectionIdentifier = kAudioServiceQueueIdentifier;
            }
        } else {
            queueCountValue = nil;
            queueIndexValue = nil;
            queueCollectionIdentifier = nil;
        }
        [self updateNowPlayingInfo];
        result(@{});
...
- (void) updateNowPlayingInfo {
    ...
    updated |= [self updateNowPlayingField:MPNowPlayingInfoPropertyPlaybackQueueCount value:queueCountValue];
    updated |= [self updateNowPlayingField:MPNowPlayingInfoPropertyPlaybackQueueIndex value:queueIndexValue];
    if (@available(iOS 9.3, macOS 10.12.2, *)) {
        updated |= [self updateNowPlayingField:MPNowPlayingInfoCollectionIdentifier value:queueCollectionIdentifier];
    }

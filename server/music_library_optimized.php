...
// 新增：全局索引文件路径
const INDEX_FILE = MUSIC_ROOT . '/.misuzu_index.json';
const SCAN_LOCK_FILE = MUSIC_ROOT . '/.scanning.lock';
...
/**
 * 优化版本：直接读取全局索引，避免逐个文件处理
 */
function handleListOptimized(): void
{
    $index = loadGlobalIndex();

    if ($index === null) {
        // 如果没有索引文件，触发初次扫描
        if (triggerBackgroundScan()) {
            respondJson(202, '正在初始化音乐库，请稍后重试', [
                'scanning' => true,
                'tracks' => []
            ]);
        } else {
            respondJson(500, '无法启动音乐库扫描');
        }
        return;
    }
    ...
    if (shouldRefreshIndex($index)) {
        triggerBackgroundScan(); // 异步更新，不阻塞当前请求
    }

    respondJson(200, 'ok', [
        'tracks' => $index['tracks'] ?? [],
        'last_scan' => $index['last_scan'] ?? null,
        'track_count' => count($index['tracks'] ?? []),
        'scanning' => isScanning()
    ]);
}
...
{
    if (triggerBackgroundScan()) {
        respondJson(200, '扫描已启动', ['scanning' => true]);
    } else {
        respondJson(409, '扫描正在进行中或无法启动');
    }
}
 ...
 * 加载全局索引文件
 */
function loadGlobalIndex(): ?array
{
 ...
 * 保存全局索引文件
 */
function saveGlobalIndex(array $index): bool
{
    $content = json_encode($index, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    return @file_put_contents(INDEX_FILE, $content, LOCK_EX) !== false;
}

/**
 * 检查是否需要刷新索引
 */
function shouldRefreshIndex(array $index): bool
{
    $lastScan = $index['last_scan'] ?? 0;
    $now = time();
    ...
    // 检查是否有新文件
    return hasNewFiles($index);
}

/**
 * 检查是否有新文件
 */
function hasNewFiles(array $index): bool
{
    $lastScan = $index['last_scan'] ?? 0;
    $indexedPaths = array_column($index['tracks'] ?? [], 'relative_path');
    ...
    try {
        ...
        foreach ($iterator as $info) {
            ...
            if (!in_array($ext, AUDIO_EXTENSIONS, true)) continue;

            $relativePath = relativePath($info->getPathname());

            // 检查是否为新文件
            if (!in_array($relativePath, $indexedPaths, true)) {
                return true;
            }

            // 检查是否为修改过的文件
            if ($info->getMTime() > $lastScan) {
                return true;
            }
        }
    } catch (Exception $e) {
        return true; // 出错时保守地返回需要更新
    }
 ...
 * 触发后台扫描
 */
function triggerBackgroundScan(): bool
{
    if (isScanning()) {
        return false;
    }
    ...
    // 创建锁文件
    if (@file_put_contents(SCAN_LOCK_FILE, time()) === false) {
        return false;
    }

    // 在后台执行扫描
    $cmd = sprintf(
        'php %s scan_background > /dev/null 2>&1 &',
 ...
 * 检查是否正在扫描
 */
function isScanning(): bool
{
    if (!is_file(SCAN_LOCK_FILE)) {
        return false;
    }

    $lockTime = (int)@file_get_contents(SCAN_LOCK_FILE);
    $now = time();
    ...
    try {
        ...
        foreach ($iterator as $info) {
            ...
            if ($processedCount % 50 === 0) {
                updateScanProgress($processedCount, count($tracks));
            }
        }

        // 排序
        usort($tracks, static fn($a, $b) => strcmp(
 ...
 * 更新扫描进度（可选实现）
 */
function updateScanProgress(int $processed, int $found): void
{
    $progress = [
        'processed' => $processed,
        'found' => $found,
    ...
    @file_put_contents(
        MUSIC_ROOT . '/.scan_progress.json',
        json_encode($progress)
    );
}

// 处理命令行调用
if (php_sapi_name() === 'cli' && isset($argv[1]) && $argv[1] === 'scan_background') {
    performFullScan();
    exit(0);
}
...
// 为了兼容性，保留原有的元数据处理函数
// (省略了buildMetadata, ensureArtwork, loadTrackMetadata等函数，与原文件相同)
...
function libraryStats(): array
{
    ...
    if ($index !== null) {
        return [
            'path' => MUSIC_ROOT,
            'audio_count' => $index['track_count'] ?? 0,
            'total_files' => $index['track_count'] ?? 0, // 简化统计
            'audio_bytes' => 0, // 可以从索引中计算
        ];
    }
...
function listAudioRelativePaths(int $limit = 200): array
{
    $index = loadGlobalIndex();
    if ($index !== null) {
        $tracks = array_slice($index['tracks'] ?? [], 0, $limit);
        return array_column($tracks, 'relative_path');
    }
    ...
    try {
        ...
        foreach ($iterator as $info) {
            ...
            if ($limit > 0 && count($paths) >= $limit) break;
        }
    } catch (UnexpectedValueException) {
        return [];
    }
...
{
    $title = 'Misuzu Music Library (优化版)';
...
<head>
    ...
    <style>
        ...
        .status.scanning { background:#fff4e6; border:1px solid #ffa940; color:#ad6800; }
        .status.ready { background:#f6ffed; border:1px solid #52c41a; color:#389e0d; }
    </style>
</head>
<body class="ms-Fabric">
    ...
    <main>
        <div class="card">
            ...
            <div class="actions" style="margin-top:12px;">
                ...
                <button id="scanBtn" class="ms-Button"><span class="ms-Button-label">重新扫描</span></button>
                <button id="forgetBtn" class="ms-Button"><span class="ms-Button-label">清除记忆</span></button>
            </div>
            <div id="statusInfo" class="status hidden"></div>
    ...
    <script>
        ...
        scanBtn.addEventListener('click', async () => {
            ...
            try {
                const resp = await fetch(window.location.pathname + '?action=scan&code=' + encodeURIComponent(code));
                const data = await resp.json();
                if (data.success) {
                    showStatus('扫描已启动，请稍后刷新', 'scanning');
                } else {
        ...
        uploadForm.addEventListener('submit', async (evt) => {
            ...
            try {
                ...
                // 稍后自动重新加载
                setTimeout(() => fetchTracks(code), 2000);
                uploadForm.reset();
            } catch (err) {
        ...
        async function fetchTracks(code) {
            ...
            try {
                ...
                if (data.scanning) {
                    showStatus('正在扫描音乐库，数据可能不完整', 'scanning');
                } else if (data.last_scan) {
                    const lastScan = new Date(data.last_scan * 1000).toLocaleString();
                    showStatus(`库已就绪，最后扫描: ${lastScan}`, 'ready');
                }
